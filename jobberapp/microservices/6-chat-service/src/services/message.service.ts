import { MsExchangeNames, MsRoutingKeys } from '@chat/enums';
...
import { IConversationDocument, IMessageDetails, IMessageDocument, lowerCase } from '@danieltalx/jobber-shared';
...
const addMessage = async (data: IMessageDocument): Promise<IMessageDocument> => {
  ...
  if (data.hasOffer) {
    ...
    await publishDirectMessage(
      chatChannel,
      MsExchangeNames.JobberOrderNotification,
      MsRoutingKeys.OrderEmail,
...
const updateOffer = async (messageId: string, type: 'accepted' | 'cancelled'): Promise<IMessageDocument> => {

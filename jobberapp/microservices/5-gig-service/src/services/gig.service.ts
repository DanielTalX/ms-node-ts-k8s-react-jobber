...
import { IRatingTypes, IReviewMessageDetails, ISellerDocument, ISellerGig } from '@danieltalx/jobber-shared';
...
import { MsElasticIndexes, MsExchangeNames, MsRoutingKeys } from '@gig/enums';

const getGigById = async (gigId: string): Promise<ISellerGig> => {
  const gig: ISellerGig = await getIndexedData(MsElasticIndexes.gigs, gigId);
  return gig;
};
...
const createGig = async (gig: ISellerGig): Promise<ISellerGig> => {
  ...
  if (createdGig) {
    ...
    await publishDirectMessage(
      gigChannel,
      MsExchangeNames.JobberSellerUpdate,
      MsRoutingKeys.UserSeller,
      JSON.stringify({ type: 'update-gig-count', gigSellerId: `${data.sellerId}`, count: 1 }),
      'Details sent to users service.'
    );
    await addDataToIndex(MsElasticIndexes.gigs, `${createdGig._id}`, data);
  }
  return createdGig;
};
...
const deleteGig = async (gigId: string, sellerId: string): Promise<void> => {
  ...
  await deleteIndexedData(MsElasticIndexes.gigs, `${gigId}`);
};
...
const updateGig = async (gigId: string, gigData: ISellerGig): Promise<ISellerGig> => {
  ...
  if (document) {
    ...
    await updateIndexedData(MsElasticIndexes.gigs, `${document._id}`, data);
  }
  return document;
};
...
const updateGigReview = async (data: IReviewMessageDetails): Promise<void> => {
  ...
  if (gig) {
    ...
    await updateIndexedData(MsElasticIndexes.gigs, `${gig._id}`, data);
  }
};

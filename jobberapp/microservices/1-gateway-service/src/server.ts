...
import { CustomError, IErrorResponse, winstonLogger } from '@danieltalx/jobber-shared';
...
export class GatewayServer {
  ...
  private errorHandler(app: Application): void {
    ...
    app.use((error: IErrorResponse, _req: Request, res: Response, next: NextFunction) => {
      ...
      if (isAxiosError(error)) {
        log.log('error', `${config.MS_NAME} Axios Error - ${error?.response?.data?.comingFrom}:`, error);
  ...
  private async startServer(app: Application): Promise<void> {
    try {
      ...
      log.log('error', `${config.MS_NAME} startServer() error method:`, error);
    }
  }

...
export class GatewayCache {
  ...
  public async saveUserSelectedCategory(key: string, value: string): Promise<void> {
    try {
      ...
      log.log('error', `${config.MS_NAME} Cache saveUserSelectedCategory() method error:`, error);
    }
  }
  ...
  public async saveLoggedInUserToCache(key: string, value: string): Promise<string[]> {
    try {
      ...
      log.log('error', `${config.MS_NAME} Cache saveLoggedInUserToCache() method error:`, error);
      return [];
    }
  }
  ...
  public async getLoggedInUsersFromCache(key: string): Promise<string[]> {
    try {
      ...
      log.log('error', `${config.MS_NAME} Cache getLoggedInUsersFromCache() method error:`, error);
      return [];
    }
  }
  ...
  public async removeLoggedInUserFromCache(key: string, value: string): Promise<string[]> {
    try {
      ...
      log.log('error', `${config.MS_NAME} Cache removeLoggedInUserFromCache() method error:`, error);
      return [];
    }
  }
}
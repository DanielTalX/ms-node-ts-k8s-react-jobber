...
import { IAuth } from '@danieltalx/jobber-shared';
...
class AuthService {
  ...
  constructor() {
    // For functionalities that do not rely on accessing protected routes
    this.axiosService = new AxiosService(`${config.AUTH_BASE_URL}/api/v1/auth`, 'auth');
    // for accessing protected routes that require the user to be logged in. This instance adds the bearer token for authorization. 

...
import { BadRequestError, IAuthDocument, IEmailMessageDetails, lowerCase } from '@danieltalx/jobber-shared';
import { Request, Response } from 'express';
import { StatusCodes } from 'http-status-codes';
import { config } from '@auth/config';
import { publishDirectMessage } from '@auth/queues/auth.producer';
import { authChannel } from '@auth/server';
import { MsEmailTypes, MsExchangeNames, MsRoutingKeys } from '@auth/enums';

/*This method is in a protected route where if the token is invalid, an error will be returned.
Valid tokens will return the user data.
This is used to verify if the user is authenticated and check token expiration.
Making a request to another page will validate the token by retrieving data from the database.*/
...
export async function resendEmail(req: Request, res: Response): Promise<void> {
  ...
  const messageDetails: IEmailMessageDetails = {
    ...
    template: MsEmailTypes.verifyEmail
  };
  await publishDirectMessage(
    authChannel,
    MsExchangeNames.JobberEmailNotification,
    MsRoutingKeys.AuthEmail,
